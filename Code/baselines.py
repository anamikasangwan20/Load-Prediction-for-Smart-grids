...
def combine_daily_data (load_data, T):
    # the input i.e. load_data is expected to be 3D [no_consumers x no_days x readings_per_day], T = no_of_days
...
def mape_(forecast, actual):
    ...
    avg_mape = np.mean(mape)                                            # averages mape values for all training samples
    return (avg_mape)
